...
export default function Example() {
  ...
  if (!state.product.name) {
    return (
      <p
        onClick={() => navigate("/")}
        className="flex justify-center align-middle items-center my-56 text-2xl text-primary font-bold cursor-pointer hover:text-secondary"
      >
        No product selected, Go back home
      </p>
    );
  } else
    return (
            ...
            <div className="mt-10 px-4 sm:px-0 sm:mt-16 lg:mt-0 text-center">
              ...
              <div className="mt-3">
                ...
                <p className="text-3xl text-gray-900">
                  {state.product?.location === "UK"
                    ? EUFormat.format(state.product?.price)
                    : NGFormat.format(state.product?.price)}
                </p>
              </div>
              ...
              <form className="mt-6">
                {state.product?.inStock ? (
                  <div className="mt-10 flex flex-col align-center items-center">
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  <div>
                    <button
                      ...
                      onClick={() =>
                        toast.warn(
                          `Sorry,${state.product?.name} is out of stock`
                        )
                      }
